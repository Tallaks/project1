...
GraphPR::GraphPR(QWidget *parent) :
    QDialog(parent)
{
    //
    // Set up the GUI
    ...
    setWindowTitle("Угловая скорость Приборная СК");
...
//
void GraphPR::onMouseUsageChanged(int mouseUsage)
{
...
//
void GraphPR::onUpdatePeriodChanged(QString text)
{
...
//
void GraphPR::onDataTimer(double x,double y,double z,double tau)
{
        ...
        m_dataSeriesA[m_currentIndex] = x;
        m_dataSeriesB[m_currentIndex] = y;
        m_dataSeriesC[m_currentIndex] = z;
        ++m_currentIndex;
...
//
void GraphPR::onChartUpdateTimer()
{
    if (m_currentIndex > 0)
    {
        //
...
//
void GraphPR::onViewPortChanged()
{
...
//
void GraphPR::onHScrollBarChanged(int value)
{
...
//
void GraphPR::updateControls(QChartViewer *viewer)
{
...
// Draw chart
//
void GraphPR::drawChart(QChartViewer *viewer)
{
    ...
    c->addTitle("    Угловая скорость в системе Прибора", "arial.ttf", 18);
    ...
    layer->addDataSet(viewPortDataSeriesA, 0xff0000, "X");
    layer->addDataSet(viewPortDataSeriesB, 0x00cc00, "Y");
    layer->addDataSet(viewPortDataSeriesC, 0x0000ff, "Z");
...
//
void GraphPR::onMouseMovePlotArea(QMouseEvent *)
{
...
//
void GraphPR::trackLineLabel(XYChart *c, int mouseX)
{
